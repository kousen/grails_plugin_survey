<div id="plugin${plugin.id}" title="You must log in to vote">
  <% def dynamic = !session.voted[plugin.name] %>
  <jsec:isNotLoggedIn>
  	<% dynamic = false %>
  	<resource:tooltip />
  	<richui:tooltip id="plugin${plugin.id}" />
  </jsec:isNotLoggedIn>
  <richui:rating dynamic="${dynamic.toString()}" id="${plugin.id}" 
  	units="5" rating="${rating}" updateId="plugin${plugin.id}" 
